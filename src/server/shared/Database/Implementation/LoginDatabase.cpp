...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(LOGIN_GET_REALMLIST, "SELECT id, name, address, port, icon, realmflags, timezone, allowedSecurityLevel, population, realmbuilds FROM realmlist WHERE realmflags <> 3 ORDER BY name", CONNECTION_SYNCH)
    ...
    PREPARE_STATEMENT(LOGIN_GET_SESSIONKEY, "SELECT a.sessionkey, a.id, af.Security  FROM account a LEFT JOIN account_forcepermission af ON (a.id = af.AccountID) WHERE username = ?", CONNECTION_SYNCH)
    ...
    PREPARE_STATEMENT(LOGIN_GET_LOGONCHALLENGE, "SELECT a.sha_pass_hash, a.id, a.locked, a.last_ip, af.Security, a.v, a.s FROM account a LEFT JOIN account_forcepermission af ON (a.id = af.AccountID) WHERE a.username = ?", CONNECTION_SYNCH)
    ...
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_ACCESS, "DELETE FROM account_forcepermission WHERE AccountID = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_ACCESS_BY_REALM, "DELETE FROM account_forcepermission WHERE AccountID = ? AND (realmID = ? OR realmID = -1)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_ACCESS, "INSERT INTO account_forcepermission (AccountID, Security, realmID) VALUES (?, ?, ?)", CONNECTION_ASYNC);
}
